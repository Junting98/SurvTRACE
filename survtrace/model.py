...
class SurvTraceSingle(BaseModel):
    ...
    def __init__(self, config):
        ...
        self.use_gpu = False
        self.linear1 = nn.Linear(self.config.num_feature * self.config.hidden_size, 1)
        self.linear2 = nn.Linear(self.config.num_feature * self.config.hidden_size, 1)
    ...
    def forward(
        ...
        return sequence_output, predict_logits, self.linear1(encoder_outputs[0].flatten(start_dim=1)).squeeze(), self.linear2(encoder_outputs[0].flatten(start_dim=1)).squeeze()
